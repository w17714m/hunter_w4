...
Pipeline de scraping y filtrado de ofertas de empleo desde Elempleo, LinkedIn y Computrabajo. Extrae descripciones con Playwright, filtra por fecha, idioma y skills, evalúa similitud con tu perfil vía embeddings y LLM, y notifica por Telegram exportando a Markdown.
...
> En particular:
>
> - **Términos de servicio**: El uso de este software puede violar los Términos de Servicio de LinkedIn, Elempleo, Computrabajo u otras plataformas. El autor no apoya ni fomenta dicha violación.
...
- [Configuración](#configuración)
  ...
  - [Archivo principal (settings.yaml)](#3-archivo-principal--configsettingsyaml)
    ...
    - [browser](#browser)
    - [stealth](#stealth)
- [Sesión de LinkedIn](#sesión-de-linkedin)
...
Lista de búsquedas a ejecutar en [Elempleo](https://www.elempleo.com). Cada elemento es un objeto con los siguientes campos:

| Campo | Tipo | Obligatorio | Descripción |
|---|---|---|---|
| `cargo` | `string` | Sí (o `url_directa`) | Texto de búsqueda, ej: `"desarrollador backend"` |
| `url_directa` | `string` | Sí (o `cargo`) | URL directa de búsqueda de Elempleo. Alternativa a construir la URL por parámetros |
...
Lista de búsquedas a ejecutar en [Computrabajo Colombia](https://co.computrabajo.com). Cada elemento:
...
| `tipo_contrato` | `string \| null` | No | Tipo de contrato (solo uno, a diferencia de Elempleo) |
...
Lista de búsquedas a ejecutar en LinkedIn Jobs. Requiere sesión autenticada (ver [Sesión de LinkedIn](#sesión-de-linkedin)). Cada elemento:

| Campo | Tipo | Obligatorio | Descripción |
|---|---|---|---|
| `cargo` | `string` | Sí | Texto de búsqueda, ej: `"backend developer"` |
| `geo_id` | `string \| null` | Sí (o `ubicacion`) | ID geográfico de LinkedIn. `"100876405"` = Colombia |
...
| `easy_apply` | `bool` | No | `true` = solo ofertas con postulación rápida. Default: `false` |
...
> **Nota**: LinkedIn ignora el filtro de tiempo en páginas de resultados profundas (más allá de la página 5-6). El `DateFilter` del pipeline (`filtros.max_days`) descarta las que superen el límite configurado antes de guardarlas.
...
| Valor | Descripción |
|---|---|
| `"relevancia"` | Por relevancia (default de LinkedIn) |
...
LinkedIn requiere autenticación. Ejecuta este script **una sola vez** antes de la primera corrida:

```bash
uv run python scripts/login_linkedin_persistent.py
```

Se abre un navegador Chromium: inicia sesión manualmente con tu cuenta de LinkedIn. El perfil de sesión queda guardado en `data/playwright/linkedin-profile/` y se reutiliza en todas las corridas siguientes.

Si LinkedIn bloquea la sesión (aparece `authwall` en los logs), vuelve a ejecutar el script.
...
```
Scraping (Elempleo + LinkedIn + Computrabajo)
    ↓
Dedup por URL              — omite ofertas ya visitadas en ciclos anteriores (consulta SQLite)
    ↓
Paginación adaptativa      — LinkedIn sigue paginando hasta encontrar 150 URLs nuevas
    ↓
