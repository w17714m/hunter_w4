...
# Mapeo f_WT: modalidad de trabajo
...
# Mapeo f_TPR: tiempo desde publicación (en segundos con prefijo 'r')
...
class LinkedinBusqueda(BaseModel):
  ...
  def validate_modalidades(cls, v: list[str]) -> list[str]:
    ...
    if invalidas:
      raise ValueError(f'modalidades inválidas: {invalidas}. Válidas: {list(LINKEDIN_MODALIDAD_MAP)}')
    return v
  ...
  def validate_tipos_contrato(cls, v: list[str]) -> list[str]:
    ...
    if invalidos:
      raise ValueError(f'tipos_contrato inválidos: {invalidos}. Válidos: {list(LINKEDIN_TIPO_CONTRATO_MAP)}')
    return v
...
# ---------------------------------------------------------------------------
# Computrabajo Colombia — mapas de filtros numéricos y slugs de path
# ---------------------------------------------------------------------------

# cont= (tipo de contrato)
...
# sal= (salario mínimo mensual en COP)
...
# pubdate= (antigüedad de publicación)
...
# jornada — segmento de path (se interpola como "jornada-{slug}")
...
# lugar_trabajo — segmento de path sufijo (se añade como "en-{slug}")
