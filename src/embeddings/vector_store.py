...
# embedding: dynamic-dimension vector (inferred when the table is created).
# id and text are the only mandatory fields for all tables.
_BASE_FIELDS = [
...
class LanceDbStore:
  ...
  Each namespace is an independent table within the same directory.
  Supports upsert by id and cosine similarity search.
  """
  ...
  def upsert_batch(self, namespace: str, docs: list[tuple[str, str]]) -> None:
    """Upsert multiple (id, text) pairs with a single batch call to the embedder."""
    if not docs:
      return
    ids, texts = zip(*docs)
  ...
  def search(
    ...
    Returns a list of dicts with keys: id, text, score (cosine distance, lower = more similar).
    Returns [] if the namespace does not yet exist.
    """
  ...
  def exists(self, namespace: str, doc_id: str) -> bool:
    """Return True if a document with the given id is already indexed."""
    if namespace not in self._db.list_tables().tables:
      return False
    table = self._db.open_table(namespace)
  ...
  def count(self, namespace: str) -> int:
    """Number of documents in the namespace. 0 if it does not exist."""
    if namespace not in self._db.list_tables().tables:
      return 0
