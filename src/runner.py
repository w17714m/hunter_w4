...
async def _collect_all(
    ...
    """Lanza los tres collectors y retorna lista de (raw_offer, source).

    url_filter omite URLs ya procesadas antes de navegar a la página de detalle.
    max_per_source limita cuántas ofertas se extraen por fuente (útil para --trial).
    """
...
async def run_once() -> None:
    ...
    logger.info('Total ofertas crudas recibidas: %d', len(raw_pairs))

    counts: dict[str, int] = {}
    for raw, source in raw_pairs:
        # Filtra resultados con extraction_status no recuperable
        status = raw.get('extraction_status', 'ok')
        if status in ('login_required', 'blocked', 'captcha', 'empty_description'):
            logger.warning('[%s] Oferta omitida por estado: %s | %s', source, status, raw.get('url', ''))
            continue

        result = pipeline.process(raw, source)  # type: ignore[arg-type]
        counts[result.estado_final] = counts.get(result.estado_final, 0) + 1

    logger.info('Ciclo completado. Resumen: %s', counts)


async def run_trial() -> None:
    """Ciclo de prueba: 1 oferta por fuente, cascade con logs detallados, sin scheduler."""
    cfg = get_config()
    # trial no usa url_filter — permite reprocesar para inspeccionar el flujo completo
    pipeline = _build_pipeline(cfg)

    logger.info('=== MODO TRIAL — 1 oferta por fuente, flujo completo ===')
    raw_pairs = await _collect_all(cfg, max_per_source=1)
    logger.info('Total ofertas crudas para trial: %d', len(raw_pairs))

    for i, (raw, source) in enumerate(raw_pairs, start=1):
        url = raw.get('url', 'sin-url')
        status = raw.get('extraction_status', 'ok')
        logger.info('--- [%d/%d] %s | %s | estado extracción: %s', i, len(raw_pairs), source, url, status)

        if status in ('login_required', 'blocked', 'captcha', 'empty_description'):
            logger.warning('  → Omitida (extracción no recuperable)')
            continue

        result = pipeline.process(raw, source)  # type: ignore[arg-type]
        logger.info('  → Estado final: %s | skipped: %s', result.estado_final, result.skipped)

    logger.info('=== TRIAL COMPLETADO ===')
...
def _run_scheduler(interval_minutes: int) -> None:
    try:
        ...
        logger.error('APScheduler no está instalado. Instale con: uv add apscheduler')
        sys.exit(1)
