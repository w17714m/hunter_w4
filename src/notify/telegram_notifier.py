...
class TelegramNotifier:
    """Send match notifications to a Telegram chat.

    Requires only token and chat_id. Has no knowledge of the pipeline or collectors.
    Network and HTTP errors are re-raised as TelegramNotifierError.
    """
    ...
    def notify(self, offer: Offer) -> None:
        """Send the offer notification. Raises TelegramNotifierError on failure."""
        if offer.veredicto is None:
            raise TelegramNotifierError(
                f'Offer {offer.id[:12]} has no verdict; cannot send notification.'
            )
    ...
    def _send(self, text: str, *, _retry: bool = True) -> None:
        ...
        try:
            ...
            raise TelegramNotifierError(f'Network error contacting Telegram: {exc}') from exc

        if resp.status_code == 429:
            retry_after = int(resp.headers.get('Retry-After', 5))
            if _retry and self.retry_on_rate_limit:
                logger.warning('Telegram rate limit. Retrying in %ds...', retry_after)
        ...
        if resp.status_code != 200:
            raise TelegramNotifierError(
                f'Telegram responded {resp.status_code}: {resp.text[:200]}'
            )
