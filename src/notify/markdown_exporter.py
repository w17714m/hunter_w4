...
def _slug(text: str) -> str:
    """Convert text to a slug of up to 40 chars suitable for file names."""
    text = text.lower()
...
class MarkdownExporter:
    """Write one .md file per offer that passed the match.

    Filename: {YYYY-MM-DD}_{offer.id[:12]}_{slug_titulo}.md
    ...
    def export(self, offer: Offer) -> Path:
        """Write the file and return its Path. Raises MarkdownExporterError on failure."""
        ...
        logger.info('Offer exported: %s', path)
        return path
