...
# ---------------------------------------------------------------------------
# Selectores — listado de resultados
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Selectores — página de detalle de oferta (DOM real 2025)
# ---------------------------------------------------------------------------
# Contenedor principal de la oferta: div[description-offer] (atributo, no clase)
# Dentro: div[div-link="oferta"] contiene todo el contenido de la vacante.
#
# Estructura interna de div[div-link="oferta"]:
#   h3.fwB.fs18.mb20           → "Descripción de la oferta" (encabezado)
#   div.mbB > span.tag.base    → tags de metadata (salario, contrato, jornada, modalidad)
#   p.mbB                      → texto principal de la descripción (con <br>)
#   ul.disc.mbB > li           → lista de requerimientos
#   p.fc_aux.fs13.mbB.mtB      → palabras clave
#
# El contenedor de empresa: div[div-link="empresa"] > p.fs18.mb20.fwB
OFFER_DETAIL_CONTAINER = '[description-offer]'
OFFER_CONTENT_SECTION = '[div-link="oferta"]'

# Selectores de metadata en el listado (article.box_offer)
# Título del puesto: h2 > a.js-o-link (en el listado)
# Empresa: p.dFlex.vm_fx > a[offer-grid-article-company-url]
# Ubicación: p.fs16.fc_base.mt5 > span.mr10

# Selectores para extracción en la página de detalle
TITLE_SELECTORS = [
...
class ComputrabajoParser:
  def extract_offer_links(self, html: str) -> list[str]:
    ...
    # Estrategia 1: artículos reales en el grid de ofertas
    # Estructura: article.box_offer[data-id] > h2 > a.js-o-link
  ...
  def parse_offer_detail(self, html: str) -> dict[str, str | None]:
    """Extrae título, empresa, ubicación y descripcion_md del HTML de una página de detalle.

    Estructura real del DOM (co.computrabajo.com 2025):
      [description-offer] > [div-link="oferta"]
        span.tag.base        → salario, contrato, jornada, modalidad
        p.mbB                → texto principal (con <br>)
        ul.disc.mbB > li     → requerimientos
      [description-offer] > [div-link="empresa"]
        p.fs18.mb20.fwB      → "Acerca de EMPRESA" (contiene el nombre)
    """
    from bs4 import BeautifulSoup
    ...
    # ---- Título: h1 o la primera h3 real del bloque de contenido ----
    titulo: str | None = None
    h1 = soup.select_one('h1')
    if h1:
      titulo = h1.get_text(' ', strip=True) or None

    # ---- Empresa: desde div[div-link="empresa"] ----
    ...
    if empresa_section:
      ...
      if empresa_p:
        ...
        # Quitar prefijo "Acerca de " si existe
        empresa = re.sub(r'^Acerca de\s+', '', raw, flags=re.IGNORECASE).strip() or None

    # ---- Ubicación: span.mr10 dentro del article del listado o de la página ----
  ...
  def _build_description_markdown(self, content: Any) -> str:
    """Construye un Markdown limpio desde el bloque de contenido de la oferta.

    Secciones extraídas en orden:
      1. Tags de metadata (salario, contrato, jornada, modalidad)
      2. Texto principal de descripción (p.mbB)
    ...
    # 2. Texto principal de la descripción — p.mbB (primer párrafo largo)
  ...
  def description_html_to_markdown(self, html: str) -> str:
    """Convierte HTML arbitrario a Markdown (usado como fallback)."""
  ...
  async def _fetch_offer(self, page: Any, offer_url: str) -> dict[str, Any]:
    try:
      ...
      if not parsed['descripcion_md']:
        return self._build_error_result(offer_url, 'empty_description', 'Markdown vacío tras extracción')
