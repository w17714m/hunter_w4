...
def test_is_seen_with_invalid_url_returns_false() -> None:
    # canonicalize_url añade https:// a cadenas sin esquema, por lo que solo
    # una cadena vacía lanza ValueError y cortocircuita la consulta a DB.
...
def test_bulk_seen_empty_string_only_no_query() -> None:
    # Solo la cadena vacía lanza ValueError en canonicalize_url y se omite.
    # Cadenas con texto (ej. 'no-es-url') reciben https:// y generan un ID válido.
    repo = _make_repo(has_any=set())
    f = URLSeenFilter(repo)
    seen = f.bulk_seen('elempleo', [''])
    assert seen == set()
