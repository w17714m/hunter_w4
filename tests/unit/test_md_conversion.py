"""HTML → Markdown conversion tests for the elempleo and LinkedIn collectors.

Verifies that the output:
  - Contains no HTML tags.
  - Contains no content from <nav>, <footer>, <script>, <header>.
  - Preserves relevant offer text.
  - Contains skills searchable in the resulting Markdown.
"""
...
# ---------------------------------------------------------------------------
# Fixtures HTML — LinkedIn (simulating the extracted description block)
# ---------------------------------------------------------------------------
...
class TestElempleoHtmlToMarkdown:
    ...
    def test_removes_header_content(self) -> None:
        result = ElempleoParser.html_to_markdown(ELEMPLEO_HTML_FULL)
        # The <header> content must have been removed
        # "Inicio | Buscar | Login" is inside <nav> within <header>
        assert 'Inicio | Buscar | Login' not in result
    ...
    def test_skills_are_searchable_in_output(self) -> None:
        ...
        for skill in ['Python', 'FastAPI', 'Docker', 'AWS', 'PostgreSQL']:
            assert skill in result, f'Skill "{skill}" not found in Markdown'
...
class TestLinkedInHtmlToMarkdown:
    ...
    def test_bold_emphasis_converted_to_markdown(self) -> None:
        ...
        # skill must be present regardless of whether bold is converted or stripped
        assert 'Python' in result
