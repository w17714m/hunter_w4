"""Unit tests for PipelineGraph.

Each test verifies that the offer ends in the correct state
according to the cascade layer that discards it. All external
services (DB, embeddings, judge, Telegram) are replaced with stubs.
"""
...
def _make_offer_model() -> Offer:
    """Minimal Pydantic Offer for LangGraph node tests."""
    return Offer(
        id='a' * 64,
        fuente='linkedin',
...
class TestLanguageFilter:
    def test_non_allowed_language_is_discarded(self) -> None:
        # German text — clearly neither ES nor EN
        raw = _raw_offer(
            descripcion_md=(
...
# ---------------------------------------------------------------------------
# Individual LangGraph node tests (Fixes F, H, J)
# ---------------------------------------------------------------------------

class TestPersistirGuard:
    """Fix F — node_persistir rejects empty estado_final."""

    def test_persistir_raises_if_estado_vacio(self) -> None:
        """estado_final='' with valid offer → RuntimeError, not silent save."""
        pipeline = _make_pipeline()
        fake_state = {
            'raw': {}, 'source': 'linkedin',
    ...
    def test_persistir_does_not_call_save_state_on_duplicate(self) -> None:
        """Fix B: estado='duplicate' → node_persistir returns {} without calling save_state."""
        pipeline = _make_pipeline()
        fake_state = {
            'raw': {}, 'source': 'linkedin',
            'offer': _make_offer_model(),
    ...
    def test_persistir_does_not_call_save_state_when_offer_is_none(self) -> None:
        """Fix A: normalization failed → offer=None → node_persistir returns {} without DB."""
        pipeline = _make_pipeline()
        fake_state = {
            'raw': {}, 'source': 'linkedin',
            'offer': None,
...
class TestNodeSimilitud:
    """Fix H — node_similitud never compares None < float."""

    def test_none_similarity_does_not_raise_type_error(self) -> None:
        """Profile not indexed → similitud=None → offer passes without TypeError."""
        store = MagicMock()
    ...
    def test_none_similarity_stored_as_none_in_result(self) -> None:
        """similitud=None propagates in state without raising."""
        store = MagicMock()
        store.search.return_value = []
    ...
    def test_low_similarity_sets_descartada(self) -> None:
        """Low similarity → descartada=True, estado_final='low_sim'."""
        store = MagicMock()
...
class TestNodeIndexar:
    """Fix J — node_indexar catches exceptions and always returns {}."""

    def test_indexar_swallows_lancedb_exception(self) -> None:
        """Exception in store.upsert does not interrupt the graph."""
        store = MagicMock()
        store.upsert.side_effect = Exception('LanceDB down')
        pipeline = _make_pipeline(store=store)
        fake_state = {
            'raw': {}, 'source': 'linkedin',
            'offer': _make_offer_model(),
