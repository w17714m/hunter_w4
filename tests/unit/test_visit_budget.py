"""Tests unitarios de VisitBudget."""
...
def test_consume_does_not_go_below_zero() -> None:
    b = VisitBudget(1)
    b.consume()
    b.consume()  # ya agotado
    assert b.remaining() == 0
    assert b.consumed() == 1  # no supera el límite
...
def test_consume_clamps_to_limit() -> None:
    ...
    b.consume()
    b.consume()
    # intentar consumir más no rompe el conteo
    b.consume()
    assert b.consumed() == 3
    assert b.remaining() == 0
...
def test_budget_sequence_matches_expected_scenario() -> None:
    """Busqueda 1: 80 visitas. Busqueda 2: intenta 90, solo consigue 70. Busqueda 3: agotada."""
