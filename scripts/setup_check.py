"""scripts/setup_check.py — Verificación de entorno antes de correr el pipeline.

Uso:
    uv run python scripts/setup_check.py

Sale con código 0 si no hay fallos críticos (fail), 1 si hay al menos uno.
"""
...
# Añade la raíz del proyecto al path para importar src.*
...
class EnvironmentChecker:
    ...
    def check_ollama(self) -> CheckResult:
        ...
        try:
            ...
            return CheckResult(
                ...
                accion=f'Verificar que Ollama está corriendo en {self._cfg.modelos.ollama_base_url}',
            )
    ...
    def check_telegram(self) -> CheckResult:
        ...
        if not token or not chat_id:
            return CheckResult(
                ...
                accion='Añadir TELEGRAM_TOKEN y TELEGRAM_CHAT_ID en .env',
            )
        ...
        try:
            ...
            return CheckResult(
                'Telegram', 'fail',
                f'Token inválido (HTTP {resp.status_code})',
    ...
    def check_linkedin_profile(self) -> CheckResult:
        ...
        if not archivos:
            return CheckResult(
                'Perfil LinkedIn', 'warn',
                f'{profile_dir}/ existe pero está vacío',
                accion='Ejecutar: uv run python scripts/login_linkedin_persistent.py',
            )
    ...
    def check_lancedb(self) -> CheckResult:
        ...
        try:
            ...
            return CheckResult('LanceDB', 'fail', 'Escritura retornó count=0')
        except Exception as exc:
            return CheckResult(
                'LanceDB', 'fail',
                f'Error de I/O: {exc}',
    ...
    def check_skills_config(self) -> CheckResult:
        ...
        if not skills:
            return CheckResult(
                'Config skills', 'fail',
                'skills.lista está vacía',
                accion='Añadir al menos un skill en config/settings.yaml → skills.lista',
            )
    ...
    def check_profile_md(self) -> CheckResult:
        ...
        if is_placeholder:
            return CheckResult(
                'profile.md', 'warn',
                'Contiene texto placeholder — el juez LLM no tendrá contexto real',
...
def print_report(results: list[CheckResult]) -> None:
    width = 48
    print(f'\n{"=" * width}')
    print('  Job Hunter — Verificación de entorno')
    print(f'{"=" * width}\n')
    ...
    if n_fail:
        print('  Hay fallos críticos — corregir antes de correr el pipeline.')
    elif n_warn:
        print('  Advertencias presentes — el sistema puede correr con limitaciones.')
    else:
        print('  Entorno listo para producción.')
    print(f'{"=" * width}\n')
...
def main() -> int:
    try:
        ...
        print(f'\n❌ No se pudo cargar la configuración: {exc}')
